...
const SIDEBAR_BACKGROUND = "var(--sidebar-background)";
...
function SidebarInset({ className, ...props }: React.ComponentProps<"main">) {
  return (
    <main
      ...
      className={cn(
        "bg-accent relative flex w-full flex-1 flex-col",
...
function SidebarMenuSkeleton({
  ...
  // Deterministic width between 50% and 90% derived from the component id.
  const instanceId = React.useId();
  const width = React.useMemo(() => {
    const charTotal = Array.from(instanceId).reduce(
      (acc, char) => acc + char.charCodeAt(0),
      0,
    );

    const normalized = (charTotal % 41) + 50;
